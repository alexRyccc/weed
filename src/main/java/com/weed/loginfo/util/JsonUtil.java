...
public class JsonUtil {
     ...
     * @return 根据传入的对象返回变化值
     */
    public static String contrastSourceFundByBean(Object oldBean, Object newBean,int a) {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (o2 == null) {
                    ...
                    str.append("\"").append(getName).append("\"").append(":\"新增值\":").append(o2);
                    i++;
                } else {
                    if (filterCheck(cf)) {
                        String otto = contrastSourceFundByBean(o1, o2,1);
                        String getName = cf.name();
                        str.append(",").append("\"").append(getName).append("\"").append(":").append("{").append(otto).append("}");
                    } else if (filterChecksets(cf)) {
                        String list = getListupdateOnly((List) o1, (List) o2);
                        String getName = cf.name();
                        str.append(",").append("\"").append(getName).append("\"").append(":").append("{").append(list).append("}");
                    } else if (filterCheckarray(cf)) {
                        String array = judgeArray(o1, o2);
                        String getName = cf.name();
                        str.append(",").append("\"").append(getName).append("\"").append(":").append("{").append(array).append("}");
                        ...
                        if (!o1.equals(o2)) {
                            ...
                            String getName = cf.name();
                            str.append("\"").append(getName).append("\"").append(":{").append("\"旧值\":").append("\"")
                                    .append(o1).append("\"").append(",").append("\"新值\":").append("\"").append(o2).append("\"").append("}");
                            i++;
                        }
                    }
    ...
    public static <E> String getListupdateOnly(List<E> aList, List<E> bList) {
        ...
        for (int i = 0; i < bList.size(); i++) {
            if (!myListContains(aList, bList.get(i))) {
                String del ="\""+bList.get(i)+"\"";
                reduceaList.add((E) del);
            }
        }
        StringBuilder recordLog = new StringBuilder();
        if (addList.size() != 0) {
            recordLog.append("\"新增信息\":" + addList);
        }

        if (reduceaList.size() != 0) {
            recordLog.append(",").append("\"减少信息\"" +":"+ reduceaList);
        }
